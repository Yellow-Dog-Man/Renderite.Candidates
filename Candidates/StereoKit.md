# RENDERER_NAME
**Repository:** [StereoKit](https://github.com/StereoKit/StereoKit)
...
**Shading language(s):** HLSL

**Rendering paths:** Foward? (NEED TO VERIFY)

**Graphics API's:** Vulkan

**Supported platforms:** Windows, Linux, MacOS, Android

**Oldest supported hardware:** Unlisted, but appears to support quite old hardware (e.g. integrated Intel GPU's)

**VR API's:** OpenXR, so it should support all OpenXR compatible headsets (e.g. Oculus, Valve Index, HTC Vive, Pimax, Varjo, etc...)

**Activity:**
32 contributors, 1.2k stars, 100+ commits in the last year, last release was V0.3.11, but V0.4.0 is currently in pre-release and has a lot of changes (including a new render backend) which switches from DirectX 11 to Vulkan.

## AI Use
it dosn't appear that StereoKit has any AI policies or uses AI in any way, but this is something that would need to be confirmed with the developers.
...
it is more a Framework, instead of a game engine, so it is more low level and modular than something like Unity or Unreal, which means that it should be easier to integrate as a renderer.

## Potential concerns
it's immediate mode similar to IMGUI, which means that the rendering is done through submitting draw calls directly from code, which is different from the retained mode rendering used by Unity and Unreal. 
this means that developers would need to implement caching and batching systems on top of the renderer to achieve good performance, which could be a significant amount of work depending on how the renderer is designed and what features it provides.
...
📖 Shader Pipeline states HLSL but this requires more information.
**Ideal:**
- ❓Slang support
- ❓SPIR-V
...
- Supported types:
  ...
  - ⚠️Directional
- Supported features:
	- ⚠️Hard & Soft realtime shadows for for all types
	- ⚠️Multiple instances of each type
	- ⚠️Light cookies for point & spot lights
- ⚠️Control over lighting falloff
📖 these might require writing custom shaders. please consult https://stereokit.net for the documentation.
**Nice to have:**
- ✅RGB light cookies
- ✅Baked shadowmaps
...
📖 This should be possible with a combination of render textures and command buffers.
...
📖 this should be possible with HLSL shader support.
...
📖 this should be solvable via writing to a texture already, but the video decoding part is the main concern here - it needs to be efficient enough to decode high resolution videos (e.g. 4K) at high framerates (e.g. 60fps) without dropping frames, and ideally with support for hardware accelerated decoding.
libVLC is a good candidate for this since it supports a wide variety of formats and has support for hardware accelerated decoding, but the integration needs to be done in a way that allows efficient texture updates and synchronization with the rendering pipeline.
